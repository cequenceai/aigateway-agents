...
from react_agent_with_memory_ravi import create_agent_executor, AIMessage
...
class InitializeRequest(BaseModel):
    ...
    auth_header: Optional[str] = Field(
        None,
        description="Optional authorization header for MCP server. If not provided, will use MCP_AUTH_HEADER from .env if available.")
...
async def initialize_agent(request: InitializeRequest):
    ...
    try:
        ...
        auth_header = request.auth_header or MCP_AUTH_HEADER
        agent_executor = await create_agent_executor(request.mcp_url, auth_header=auth_header)
...
async def invoke_agent(
    ...
    try:
        ...
        config = {
            ...
            "recursion_limit": 100  # Increase recursion limit to handle complex tool sequences
        }

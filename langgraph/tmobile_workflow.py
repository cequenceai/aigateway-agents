...
async def list_plans_node(state: GraphState) -> GraphState:
    ...
    # 1. Create a cart in the background
    logger.info("Creating a new cart...")
    cart_params = {"Authorization": "Bearer token", "Content-Type": "application/json"}
    ...
    cart_id = json.loads(create_cart_result["result"]).get("cartId")
...
# workflow.add_node("check_eligibility", check_eligibility_node)
# workflow.add_node("get_plan_options", get_plan_options_node)
# workflow.add_node("select_plan", select_plan_node)
...
workflow.add_edge("list_plans", END)

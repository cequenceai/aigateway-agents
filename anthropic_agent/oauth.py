...
class InMemoryTokenStorage(TokenStorage):
    ...
    def __init__(self, storage_dir: str | None = None, reset_tokens: bool = False):
        ...
        # Load existing tokens from disk (unless reset_tokens=True)
        if not reset_tokens:
...
class CallbackHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if "code" in query_params:
            ...
            # Send a simple response for non-OAuth requests
            self.send_response(200)
            self.send_header("Content-Type", "text/html")
            self.end_headers()
            self.wfile.write(b"<html><body>OAuth callback server is running...</body></html>")
...
def create_oauth_provider(
    ...
    async def redirect_handler(authorization_url: str) -> None:
        ...
        # Try to open Chrome browser (preferred over Safari)
